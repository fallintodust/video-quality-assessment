...
### 3.2 FAST-VQA 系列内部对照（计算量 vs 精度）

| 模型 | MACs | SROCC | PLCC | OBJ | 单视频耗时 |
|---|---|---|---|---|---|
| FAST-VQA-B | 279 G | 0.7102 | 0.7111 | **1.4213** | 7.47 s |
| FasterVQA | 69 G | 0.6713 | 0.6770 | 1.3484 | 1.56 s |

FasterVQA 的计算量只有 FAST-VQA-B 的 1/4，速度快 4.8 倍，OBJ 低 0.073。

**这一行与任务书的 300 G FLOPs 限制直接相关**：FAST-VQA-B 的 279 G 已经贴近上限，
而 FasterVQA 的 69 G 有 4 倍余量，代价是 5% 的精度。这说明在本任务上，
计算量与精度的权衡曲线相当平缓，堆算力的收益有限。

> **待复核**：FasterVQA 这一行可能被低估。`f3dvqa-b.yml` 把 `t_frag: 8` 写在
> `sample_types.fragments` 内部而非 `args` 层级，上游 `vqa.py`（以及本脚本初版）
> 只在 `args` 层级查找，因此回退到了普通 `SampleFrames`，而 FasterVQA 是按
> 3D 片段采样训练的。脚本已修正（同时查找两个层级），重跑后此行数值应会提高。
>
> **未采用**：FAST-VQA-M（46 G）实测 SROCC 0.2970 / PLCC 0.1741 / OBJ 0.4711，
> 与论文报告的水平（PLCC@KoNViD 0.832）严重不符，判断为配置或权重问题，
> 未查明原因前不列入正式结果。
...
（见 `docs/cached_feature_pipeline2.md` 6.4 节），这个差距约为噪声的 2 倍。
...
**指标存在乐观偏差。** 我们的模型按验证集挑选最优 epoch（60 轮中取最好），
这会带来约 0.01~0.02 的乐观偏移；FAST-VQA 是零样本单次推理，无此偏差。
因此真实差距可能略大于 0.0197。组内 baseline 与半监督 v2 采用同样的选取方式，
存在同样的偏差。
...
- [ ] 用修正后的脚本重跑 FasterVQA（`t_frag` 查找修复，约 25 分钟），
      当前 1.3484 可能被低估
- [ ] 查明 FAST-VQA-M 指标异常的原因（OBJ 0.4711，与论文严重不符）
