...
# -*- coding: utf-8 -*-
"""
Benchmark noise detectors against the T-3 (noiseless) labels.

Both detectors output "severity" (1 = severe noise) while the T-3 label is
"noiselessness" (high = clean), so the two are expected to be
anti-correlated. The script reports SROCC/PLCC of -score against the label,
i.e. a positive number means the detector agrees with the human ratings.
...
NOTE: heuristic_noise is currently a placeholder (constant score), so its
rows are expected to show nan correlations until the estimator lands.

Usage:
    python3 scripts/eval_noise.py --videos ~/trkv/data --feats ~/trkv/feats \
        --labels data/divide/t3_train_lable_test.txt \
        --ckpt runs/t3/best_r50_mean+std+diff.pt
    python3 scripts/eval_noise.py ... --limit 200        # quick check
    python3 scripts/eval_noise.py ... --skip-model       # heuristic only
"""
import argparse
import os
import sys
...
from noise_detectors import heuristic_noise  # noqa: E402
...
def report(name, scores, labels, elapsed, n):
    """scores: higher = more distorted. labels: higher = more noiseless."""
    s = -np.asarray(scores)
    y = np.asarray(labels)
...
def main():
    ...
    ap.add_argument("--labels", required=True, help="T-3 (noiseless) validation labels")
    ap.add_argument("--feats", default="", help="feature cache (for the model)")
    ap.add_argument("--ckpt", default="", help="T-3 head checkpoint")
    ...
    ap.add_argument("--out", default="runs/noise_eval.json")
    args = ap.parse_args()
    ...
    # ---- heuristic: needs pixels, so it decodes (placeholder until implemented)
    if not args.skip_heuristic:
        for uniform in (False, True):
            tag = "heuristic_noise (uniform frames)" if uniform else "heuristic_noise (clips)"
    ...
    if args.model_live and args.ckpt:
        from noise_detectors import ModelNoise
        det = ModelNoise(args.ckpt)
        for uniform in (False, True):
