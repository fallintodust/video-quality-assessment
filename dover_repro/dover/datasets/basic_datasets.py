...
class Cv2VideoReader:
    """decord.VideoReader 的 cv2 替代（Windows 无 decord wheel）。

    行为对齐 decord(以 torch bridge)：__getitem__ 返回 uint8 torch.Tensor，
    形状 [H, W, 3]（RGB），__len__ 返回总帧数。
    """
    ...
    def __init__(self, path):
        ...
        # cv2 对个别视频会多报 1 帧（duration*fps 舍入），而 decord/ffmpeg 用
        # 真实可解码帧数。这里校验尾帧可读性，不一致则回退，保证与官方采样一致。
        self.cap.set(cv2.CAP_PROP_POS_FRAMES, max(0, self.n - 1))
        while self.n > 1 and not self.cap.read()[0]:
            self.n -= 1
            self.cap.set(cv2.CAP_PROP_POS_FRAMES, self.n - 1)
    ...
    def __getitem__(self, idx):
        if isinstance(idx, (slice, list, tuple, np.ndarray)):
            return [self._read(int(i)) for i in idx]
    ...
    def _read(self, i):
        ...
        if not ret:
            # 部分视频尾帧 seek 后 read 失败（容器帧计数与解码器不一致）：回退一帧
            self.cap.set(cv2.CAP_PROP_POS_FRAMES, max(0, i - 1))
            ret, frame = self.cap.read()
        assert ret, f"读取帧失败: idx={i}"
    ...
    def __getitem__(
        ...
        if tocache or self.cache is None:
            ...
            if filename.endswith(".yuv"):
                ...
                vreader = Cv2VideoReader(filename)
