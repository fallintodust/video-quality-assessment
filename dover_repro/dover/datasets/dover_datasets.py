...
from .basic_datasets import Cv2VideoReader
...
def spatial_temporal_view_decomposition(
    ...
    if video_path.endswith(".yuv"):
        ...
        vreader = Cv2VideoReader(video_path)
...
class ViewDecompositionDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if isinstance(label, tuple):
            # 官方 DOVER++ 训练需要三个分支标签：
            # 标注文件列为 [filename, aesthetic, technical, overall]
            data["gt_label_a"], data["gt_label_t"], data["gt_label"] = label
